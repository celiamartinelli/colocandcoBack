...
const articleDataMapper = {
  async getArticles(colocId) {
    const sqlQuery = 'SELECT * FROM article_shoplist WHERE colocation_id = $1';
  ...
  async addArticle(colocId, body) {
    const sqlQuery = `INSERT INTO article_shoplist (name, colocation_id)     VALUES (
      $2,
      $1
    )
  RETURNING *
`;
  ...
  async deleteArticle(articleId) {
    const sqlQuery = 'DELETE FROM article_shoplist WHERE id = $1; ';
    // eslint-disable-next-line max-len
